...
def create_pdf(smth):
  ...
  if len(mtext) > 230:
    mtext = mtext[:230]+ '...'
  mtext = re.sub(u'[^\u0020-\u00BF\u0410-\u044F]', '', mtext)
  mtype = smth['type']
  mid =  smth['id']

  # musername = 'alexandrettio'
  # mtext = 'даэтож текст!! длинный жирный текст для проверки всего что можно проверить и всего что проверить нельзя. Кроме смайликов и обрезания слишком длинного текста'
  # print(mtext)
  # mtype = 'image'
  # mid =  'testasya_test'
  ...
  if mtype != 'image':
    ...
    f["adres"] = "Юго-Западная, ул. Академика Анохина 4, корпус 3"
    f["avatar"] = "images/profile.jpg"
    f["photo"] = "images/content.jpg"
    f["username"] = musername
    f["logo"] = "images/logo.jpg"
    f["place"] = "images/place.png"
    f["clubname"] = "Time Club Коперник"
    f["phone"] = "+7(495) 215-14-87"
    f["description"] = mtext
    f.render("results-kpr/%s-5.pdf" %mid)
